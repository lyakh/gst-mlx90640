...
//------------------------- To calculation -------------------------------------
    ...
    float multiplier = (1 + params->cpKta * (ta - 25)) *
	    (1 + params->cpKv * (vdd - 3.3));

    irDataCP[0] -= params->cpOffset[0] * multiplier;

    if (mode ==  params->calibrationModeEE)
    {
        irDataCP[1] -= params->cpOffset[1] * multiplier;
    }
    else
    {
      irDataCP[1] -= (params->cpOffset[1] + params->ilChessC[0]) * multiplier;
    }
    ...
    for (int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        conversionPattern = ((pixelNumber + 2) / 4 - (pixelNumber + 3) / 4 + (pixelNumber + 1) / 4 -
        ...
        if (pattern == frameData[833])
        {
            ...
            To = sqrt(sqrt(irData/(alphaCompensated * (1 - params->ksTo[1] * 273.15) + Sx) + taTr)) -
		    273.15;
...
//------------------------- Image calculation -------------------------------------
    ...
    for (int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ilPattern = (pixelNumber >> 5) & 1; // odd or even row
        chessPattern = ilPattern ^ (pixelNumber & 1);

	/*
	 * Each subpage makes the following data available in camera RAM:
	 * (1) a half of pixel data, depending whether the chess (default) or
	 * the interleaved pattern is used, which amounts to 12 "lines" of 32
	 * 2-byte entries.
	 * (2) two "lines" (2 lines of 32 2-byte entries) of metadata
	 * That makes 14 "lines" in total.
	 *
	 * This function walks all 768 = 32*24 pixels of input data but only
	 * calculates a half of the output data
	 *
	 * The driver reads both subpages at once, so, presumably it reads the
	 * metadata from the second subpage.
	 *
	 * parameters different between subpages:
	 *
	 */

	pattern = mode ? chessPattern : ilPattern;
	    ...
	    float kv = params->kv[pixelNumber] / kvScale;
		    ...
		    (1 + kta * (ta - 25)) * (1 + kv * (vdd - 3.3));

            if (mode != params->calibrationModeEE)
            {
                int8_t conversionPattern;
/*
 * pixelNumber:		conversionPattern(ilPattern=0)	conversionPattern(ilPattern=1)
 *			row 0, 2, 4,...			row 1, 3, 5,...
 * 0			0				0
 * 1			-1				1
 * 2			0				0
 ...
 */
            ...
            result[pixelNumber] = irData * params->alpha[pixelNumber];
        }
    }
}
#else
...
//------------------------- To calculation -------------------------------------
    ...
    for (int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        irData -= params->tgc * irDataCP[chessPattern];
        irData /= emissivity;
        ...
        To = sqrt(sqrt(irData / (alphaCompensated *
				     (1 - params->ksTo[1] * 273.15) + Sx) + taTr)) - 273.15;
...
//------------------------- Image calculation -------------------------------------
    // Assume chess mode for now, print CTL1 from the driver later
    mode = 0x1000 >> 5;
	 ...
	 * row 1: 1 0 1 0...
	 * ...
	 * i.e. it's effectively pixel's subpage number
	 */
	...
	/* in chess mode subpage 0 includes chessPattern=0 pixels */
	    float irData = frameData[pixelNumber];
        if (irData > 32767)
        {
            irData -= 65536;
...
float MLX90640_GetVdd(uint16_t *frameData, const paramsMLX90640 *params)
{
    ...
    resolutionRAM = (params->resolution & 0x0C00) >> 10;
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
	...
	for (int j = 0; j < 32; j++, p++)
        {
            ...
            alphaTemp[p] -= mlx90640->tgc * (mlx90640->cpAlpha[0] + mlx90640->cpAlpha[1]) / 2;
    ...
    alphaScale = (unsigned int)log2f(2 * 32767.4 / (temp + FLT_EPSILON));
